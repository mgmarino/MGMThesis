...
\def\gersevenone{$^{71}$Ge}
\def\gersevensix{$^{76}$Ge}
\def\gerseventhree{$^{73}$Ge}
\def\galsixeight{$^{68}$Ga}
\def\asseventhree{$^{73}$As}
\def\znsixfive{$^{65}$Zn}
\def\amtwofourone{$^{241}$Am}
\def\hthree{$^{3}$H}
\def\tbf{\boldsymbol{\theta}}
\def\signuc{\sigma_{nucl}}
\def\sigman{\sigma_{W-n}}
\def\gaa{g_{a\bar{e}e}}
\def\pll{\lambda(\signuc)}
\def\plln{\lambda(N_{axion})}
\def\gpp{$g_{pp}$}                      % the g_pp of QRPA fame
\def\releff{$\rho_{eff}$}                      % the g_pp of QRPA fame
\newcommand{\qval} {Q_{\nonubb}}                   % The Q-value
\def\WIMPAnn{\langle \sigma_{A} v \rangle^{-1}}
